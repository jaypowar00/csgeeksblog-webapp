...
function AdminPage() {
    ...
    return (
        <>
            <Head>
                <title>Admin Login | CSGeeks</title>
                ...
                <meta property="og:description" content="Login as admin of CSGeeks" />
                ...
                <meta name="twitter:description" content="Login as admin of CSGeeks" />
                        ...
                        <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto h-[85vh] lg:py-0">
                            <div className="w-full rounded-lg shadow border md:mt-0 sm:max-w-md xl:p-0 bg-gray-800 border-gray-700">
                                <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                                    <h1 className="font-bold leading-tight tracking-tight md:text-2xl text-white">
                                        Signin your admin account
                                    </h1>
                                    <form className="space-y-4 md:space-y-6" onSubmit={handleLogin}>
                                        <div>
                                            <label htmlFor="email" className="block mb-2 text-sm font-medium  text-white">Username</label>
                                            <input type="text" name="username" id="username" className="bg-gray-50 border sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-blue-500 focus:border-blue-500" required
                                                value={username} onChange={e => setUsername(e.target.value)} autoComplete="username" />
                                        </div>
                                        <div>
                                            <label htmlFor="password" className="block mb-2 text-sm font-medium text-white">Password</label>
                                            <input type="password" name="password" id="password" className="mb-3 bg-gray-50 border sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-blue-500 focus:border-blue-500" required
                                                value={password} onChange={e => setPassword(e.target.value)} autoComplete="current-password" />
                                        </div>
                                        <button disabled={adminLoginStatusLoading} type="submit" className={`w-full text-white focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5 text-center ${adminLoginStatusLoading ? 'bg-gray-600 hover:bg-gray-700 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700'}`}>Sign in</button>
                                        {
                                            adminLoginStatusLoading ?
                                                <span className="text-sm text-green-400">checking authentication status...</span> : <></>
                                        }
                                        {
                                            (errorReason) ?
                                                <span className="text-sm">Error: <span className="text-red-500">{errorReason}</span></span> : <></>
                    ...
                    <footer className="bg-gray-800">
                        <div className="p-4 pr-5 py-6 mx-auto max-w-screen-xl md:p-8 lg:p-10">
                            <div className="grid grid-cols-3 gap-8 md:grid-cols-3 lg:grid-cols-4">
                                ...
                                <div>
                                    <h2 className="mb-6 text-sm font-semibolduppercase text-white">Info</h2>
                                    <ul className="text-gray-500 text-gray-400">
                                        <li className="mb-4">
